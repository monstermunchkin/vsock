...
func (s *execWs) Connect(op *operation, r *http.Request, w http.ResponseWriter) error {
	...
	for fd, fdSecret := range s.fds {
		if secret == fdSecret {
			conn, err := lxdshared.WebsocketUpgrader.Upgrade(w, r, nil)
			if err != nil {
				return err
			}
...
func (s *execWs) Do(op *operation) error {
	...
	if s.interactive {
		ttys = make([]*os.File, 1)
		ptys = make([]*os.File, 1)
		ptys[0], ttys[0], err = lxdshared.OpenPty(s.rootUid, s.rootGid)
		if err != nil {
			return err
		}
		...
		if s.width > 0 && s.height > 0 {
			lxdshared.SetSize(int(ptys[0].Fd()), s.width, s.height)
		}
	} else {
		ttys = make([]*os.File, 3)
		ptys = make([]*os.File, 3)
		for i := 0; i < len(ttys); i++ {
			ptys[i], ttys[i], err = lxdshared.Pipe()
			if err != nil {
				return err
			}
		}
		...
		go func() {
			...
			for {
				...
				if command.Command == "window-resize" {
					...
					err = lxdshared.SetSize(int(ptys[0].Fd()), winchWidth, winchHeight)
					if err != nil {
		...
		for i := 0; i < len(ttys); i++ {
			go func(i int) {
				if i == 0 {
					...
					<-lxdshared.WebsocketRecvStream(ttys[i], conn)
					ttys[i].Close()
				} else {
					s.connsLock.Lock()
					conn := s.conns[i]
					...
					<-lxdshared.WebsocketSendStream(conn, ptys[i], -1)
					ptys[i].Close()
					wgEOF.Done()
				}
			}(i)
	...
	finisher := func(cmdResult int, cmdErr error) error {
		...
		metadata := lxdshared.Jmap{"return": cmdResult}
...
func execHandler(w http.ResponseWriter, r *http.Request) Response {
	...
	for i := -1; i < len(ws.conns)-1; i++ {
		ws.fds[i], err = lxdshared.RandomCryptoString()
		if err != nil {
	...
	op, err := operationCreate("default", operationClassWebsocket, resources, ws.Metadata(), ws.Do, nil, ws.Connect)
	if err != nil {
		return InternalError(errors.Wrap(err, "OperationCreate"))
	}
